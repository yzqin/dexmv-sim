...
ASSETS_ROOT = os.path.join(os.path.dirname(__file__), "../models/assets")
RED = [1, 0, 0, 1]
GREEN = [0, 1, 0, 1]
BLUE = [0, 0, 1, 1]
CYAN = [0, 1, 1, 1]
...
def get_default_asset_dir():
    ...
    return os.path.abspath(os.path.join(current_dir, "../models/assets"))
...
def array_to_string(array):
    ...
    if isinstance(array, Iterable):
        ...
        raise TypeError(f"Incompatible type {type(array)} used in mujoco xml")
...
def string_to_array(string):
    ...
    if isinstance(string, (float, list)):
        ...
        array = [float(val) for val in string.split(" ")]
...
def recolor_collision_geoms(root, rgba, exclude=None):
    ...
    """
    # Check this body
    if root.tag == "geom" and root.get("group") in {None, "2"} and (exclude is None or not exclude(root)):
